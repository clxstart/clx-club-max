...
- [x] 定时任务每 5 分钟校准 Redis/DB 计数偏差
  - 实测：LikeSyncJob.sync 第 36 行 `@Scheduled(fixedRate = 5 * 60 * 1000)` ✓
...
- [x] D6 校准频率每 5 分钟 → LikeSyncJob.sync `fixedRate = 5 * 60 * 1000` ✓
...
- [x] **C13**：Redis > DB 时更新 DB
  - 验证方式：单测（LikeSyncJobTest.java:48-56）
  - 结果：通过 ✓

- [x] **C14**：Redis < DB 时更新 Redis
  - 验证方式：单测（LikeSyncJobTest.java:59-68）
  - 结果：通过 ✓

- [x] **C15**：Redis key 不存在时从 DB 初始化
  - 验证方式：单测（LikeSyncJobTest.java:71-81）
  - 结果：通过 ✓
